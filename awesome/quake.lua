...
function QuakeConsole:display()
   ...
   local fl = io.open("/tmp/lualog", "w")
   --fl:write(awful.client.cycle(true, self.screen))
   local myarray = function(c)
       return awful.rules.match(c, {instance == "terminator"})
   end
   for c in awful.client.iterate(myarray) do
       fl:write(c.name)
   end
   fl:write("blabla\n\n")
   cl = awful.client.next(1)
   fl:write(cl.instance .. "\n")
   cl = awful.client.next(2)
   fl:write(cl.instance .. "\n")
   cl = awful.client.next(3)
   fl:write(cl.instance .. "\n")
   cl = awful.client.next(4)
   fl:write(cl.instance .. "\n")
   fl:write("\n")

   fl:close()

   for c in awful.client.cycle( function (cl)
				  -- cl.name may be changed!
				  return cl.instance == self.name
			       end,
			        self.screen) do
      i = i + 1
      if i == 1 then
